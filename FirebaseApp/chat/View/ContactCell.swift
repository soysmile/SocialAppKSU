...
class ContactCell: UITableViewCell {
            ...
            ref.observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                if let dictionary = snapshot.value as? [String: AnyObject] {
                    ...
                    if let profileImageUrl = dictionary["profileImageUrl"] as? String {
                        ...
                        self.contactImage.loadImageUsingCacheWithUrlString(profileImageUrl)
                    }
                }
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        contactImage.leftAnchor.constraint(equalTo: self.leftAnchor, constant: 8).isActive = true
        contactImage.centerYAnchor.constraint(equalTo: self.centerYAnchor).isActive = true
        contactImage.widthAnchor.constraint(equalToConstant: 32).isActive = true
        contactImage.heightAnchor.constraint(equalToConstant: 32).isActive = true
